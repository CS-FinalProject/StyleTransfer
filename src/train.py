...
def models_checkpoints(real_imageA, real_imageB, args, epoch_idx: int, batch_idx: int, cycle_gan: CycleGAN,
    ...
    torch.save(cycle_gan.generator_A2B.state_dict(), os.path.join(GEN_A2B_PATH, str(counters["genA2B"]) + ".pth"))
    torch.save(cycle_gan.generator_B2A.state_dict(), os.path.join(GEN_B2A_PATH, str(counters["genB2A"]) + ".pth"))
    torch.save(cycle_gan.discriminator_A.state_dict(), os.path.join(DISC_A_PATH, str(counters["discA"]) + ".pth"))
    torch.save(cycle_gan.discriminator_B.state_dict(), os.path.join(DISC_B_PATH, str(counters["discB"]) + ".pth"))
